                            ...
                            @if($cats->isNotEmpty())
                                ...
                                <table
                                    ...
                                    <tr class="fw-bolder text-muted bg-light bd-cyan">
                                        ...
                                        <th class="min-w-150px">Category Slug</th>
                                        <th class="min-w-140px">Parent Name</th>
                                        <th class="min-w-150px">Description</th>
                                        <th class="min-w-150px">Category Image</th>
                                        <th class="min-w-120px">Display Products</th>
                                        <th class="min-w-90px">Menu</th>
                                        <th class="min-w-110px">Slider Bottom</th>
                                        <th class="min-w-90px">Featured</th>
                                        <th class="min-w-110px">Home Promo</th>
                                    ...
                                    @foreach ($cats as $cat)
                                        <tr>
                                            ...
                                            <td class="text-dark fs-6">{{ $cat->category_slug ?: '—' }}</td>
                                            <td class="text-dark fs-6">{{ $cat->parent ? $cat->parent->category_name : null }}</td>
                                            <td class="text-dark fs-6 ">{{ $cat->category_description }}</td>
                                            <td class="text-dark fs-6">
                                                @if ($cat->category_image)
                                                    <img src="{{ \App\Support\MediaStorage::url($cat->category_image, 'categories') }}"
                                                        alt="{{ $cat->category_name }}"
                                                        style="width: 70px; height: 55px; object-fit: cover; border-radius: 5px;">
                                                @else
                                                    <span class="text-muted">No image</span>
                                                @endif
                                            </td>
                                            <td>
                                                @if ($cat->is_display_products)
                                                    <span class="badge badge-light-success">Yes</span>
                                                @else
                                                    <span class="badge badge-light-danger">No</span>
                                                @endif
                                            </td>
                                            <td><span class="badge {{ $cat->is_menu ? 'badge-light-success' : 'badge-light-danger' }}">{{ $cat->is_menu ? 'Yes' : 'No' }}</span></td>
                                            <td><span class="badge {{ $cat->is_slider_bottom ? 'badge-light-success' : 'badge-light-danger' }}">{{ $cat->is_slider_bottom ? 'Yes' : 'No' }}</span></td>
                                            <td><span class="badge {{ $cat->is_feature ? 'badge-light-success' : 'badge-light-danger' }}">{{ $cat->is_feature ? 'Yes' : 'No' }}</span></td>
                                            <td>
                                                <form method="POST"
                                                    action="{{ route('category-toggle-home-promo', $cat->id) }}">
                                                    @csrf
                                                    @method('PATCH')
                                                    <div class="form-check form-switch form-check-custom form-check-solid">
                                                        <input class="form-check-input" type="checkbox"
                                                            id="homePromo{{ $cat->id }}"
                                                            aria-label="Show {{ $cat->category_name }} in home promo"
                                                            {{ $cat->is_home_promo ? 'checked' : '' }}
